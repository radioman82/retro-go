...
// PGM 01/19/97 - team color drawing
	...
	int face_x = 112;
#ifdef RG_SCREEN_WIDTH
	if (RG_SCREEN_WIDTH <= 240)
		face_x = 126;
	else if (RG_SCREEN_WIDTH < 320)
		face_x = 122;
#endif
	...
	{
		Sbar_DrawPic (face_x, 0, sb_face_invis_invuln);
		return;
	}
	if (cl.items & IT_QUAD)
	{
		Sbar_DrawPic (face_x, 0, sb_face_quad );
		return;
	}
	if (cl.items & IT_INVISIBILITY)
	{
		Sbar_DrawPic (face_x, 0, sb_face_invis );
		return;
	}
	if (cl.items & IT_INVULNERABILITY)
	{
		Sbar_DrawPic (face_x, 0, sb_face_invuln);
		return;
	}
	...
	if (cl.time <= cl.faceanimtime)
	{
		...
		anim = 0;
	Sbar_DrawPic (face_x, 0, sb_faces[f][anim]);
}
...
void Sbar_Draw (void)
{
   ...
   // armor
		if (cl.items & IT_INVULNERABILITY)
		{
			Sbar_DrawNum (armor_x, 0, 666, 3, 1);
		...
		else
		{
			if (rogue)
			{
				Sbar_DrawNum (armor_x, 0, cl.stats[STAT_ARMOR], 3,
								cl.stats[STAT_ARMOR] <= 25);
				if (cl.items & RIT_ARMOR3)
					Sbar_DrawPic (armor_x - 24, 0, sb_armor[2]);
				else if (cl.items & RIT_ARMOR2)
					Sbar_DrawPic (armor_x - 24, 0, sb_armor[1]);
				else if (cl.items & RIT_ARMOR1)
					Sbar_DrawPic (armor_x - 24, 0, sb_armor[0]);
			}
			else
			{
				Sbar_DrawNum (armor_x, 0, cl.stats[STAT_ARMOR], 3
	...
	// health
		Sbar_DrawNum (health_x, 0, cl.stats[STAT_HEALTH], 3
	...
	// ammo icon
		if (rogue)
		{
			if (cl.items & RIT_SHELLS)
				Sbar_DrawPic (ammo_icon_x, 0, sb_ammo[0]);
			else if (cl.items & RIT_NAILS)
				Sbar_DrawPic (ammo_icon_x, 0, sb_ammo[1]);
			else if (cl.items & RIT_ROCKETS)
				Sbar_DrawPic (ammo_icon_x, 0, sb_ammo[2]);
			else if (cl.items & RIT_CELLS)
				Sbar_DrawPic (ammo_icon_x, 0, sb_ammo[3]);
			else if (cl.items & RIT_LAVA_NAILS)
				Sbar_DrawPic (ammo_icon_x, 0, rsb_ammo[0]);
			else if (cl.items & RIT_PLASMA_AMMO)
				Sbar_DrawPic (ammo_icon_x, 0, rsb_ammo[1]);
			else if (cl.items & RIT_MULTI_ROCKETS)
				Sbar_DrawPic (ammo_icon_x, 0, rsb_ammo[2]);
		}
		else
		{
		...
		Sbar_DrawNum (ammo_x, 0, cl.stats[STAT_AMMO], 3,
