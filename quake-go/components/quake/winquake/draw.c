...
void Draw_Pic (int x, int y, qpic_t *pic)
{
	...
	if (x1 < 0) x1 = 0;
	if (y1 < 0) y1 = 0;
	if (x2 > (int)vid.width) x2 = vid.width;
	if (y2 > (int)vid.height) y2 = vid.height;
	...
	width = x2 - x1;
	height = y2 - y1;
	source = pic->data + (y1 - y) * pic->width + (x1 - x);

	if (r_pixbytes == 1)
	{
		dest = vid.buffer + y1 * vid.rowbytes + x1;
	...
	{
		pusdest = (unsigned short *)vid.buffer + y1 * (vid.rowbytes >> 1) + x1;
...
void Draw_Fill (int x, int y, int w, int h, int c)
{
	...
	else
	{
		...
		pusdest = (unsigned short *)vid.buffer + y1 * (vid.rowbytes >> 1) + x1;
		for (v=0 ; v<height ; v++, pusdest += (vid.rowbytes >> 1))
			for (u=0 ; u<width ; u++)
				pusdest[u] = uc;
	}
}
