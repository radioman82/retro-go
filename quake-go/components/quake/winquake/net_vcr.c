...
static long GET_SESSION(qsocket_t *sock) {
	long s;
	memcpy(&s, &sock->driverdata, sizeof(long));
	return s;
}
...
int VCR_GetMessage (qsocket_t *sock)
{
	...
	if (host_time != next.time || next.op != VCR_OP_GETMESSAGE || next.session != GET_SESSION(sock))
...
int VCR_SendMessage (qsocket_t *sock, sizebuf_t *data)
{
	...
	if (host_time != next.time || next.op != VCR_OP_SENDMESSAGE || next.session != GET_SESSION(sock))
		Sys_Error ("VCR missmatch");
...
qboolean VCR_CanSendMessage (qsocket_t *sock)
{
	...
	if (host_time != next.time || next.op != VCR_OP_CANSENDMESSAGE || next.session != GET_SESSION(sock))
		Sys_Error ("VCR missmatch");
