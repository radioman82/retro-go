...
#ifdef FCEU_LOW_RAM
#include "fceu.h"
extern uint8 FCEUI_ReadMem(uint32 A);
extern void FCEUI_WriteMem(uint32 A, uint8 V);
#endif
#include "fceu-cart.h"
...
void FCEUI_DumpVid(const char *fname, uint32 start, uint32 end) {
  RFILE *fp = filestream_open(fname, RETRO_VFS_FILE_ACCESS_WRITE,
...
void FCEUI_DumpMem(const char *fname, uint32 start, uint32 end) {
  ...
  for (; start <= end; start++)
#ifdef FCEU_LOW_RAM
    filestream_putc(fp, FCEUI_ReadMem(start));
#else
...
void FCEUI_LoadMem(const char *fname, uint32 start, int hl) {
  int t;
  RFILE *fp = filestream_open(fname, RETRO_VFS_FILE_ACCESS_READ,
                              RETRO_VFS_FILE_ACCESS_HINT_NONE);
...
static char *fstrings[12] = {"#$%02X",    /* immediate */
                             "$%04X",     /* RELATIVE(jump) */
                             "$%02X",     /* Z */
                             "$%02X,X",   /* Z,x */
                             "$%02X,Y",   /* Z,y */
...
static OPS optable[NUMOPS] = {
    ...
    {"ADC",
     1,
     {IMD(0x69), ZP(0x65), ZPX(0x75), ABS(0x6D), ABX(0x7d), ABY(0x79),
      INX(0x61), INY(0x71), -1}},
    {"AND",
     1,
     {IMD(0x29), ZP(0x25), ZPX(0x35), ABS(0x2D), ABX(0x3d), ABY(0x39),
      INX(0x21), INY(0x31), -1}},
    {"BIT", 1, {ZP(0x24), ABS(0x2c), -1}},
    {"CMP",
     1,
     {IMD(0xc9), ZP(0xc5), ZPX(0xd5), ABS(0xcD), ABX(0xdd), ABY(0xd9),
      INX(0xc1), INY(0xd1), -1}},
    {"CPX", 1, {IMD(0xe0), ZP(0xe4), ABS(0xec), -1}},
    {"CPY", 1, {IMD(0xc0), ZP(0xc4), ABS(0xcc), -1}},
    {"EOR",
     1,
     {IMD(0x49), ZP(0x45), ZPX(0x55), ABS(0x4D), ABX(0x5d), ABY(0x59),
      INX(0x41), INY(0x51), -1}},
    {"LDA",
     1,
     {IMD(0xa9), ZP(0xa5), ZPX(0xb5), ABS(0xaD), ABX(0xbd), ABY(0xb9),
      INX(0xa1), INY(0xb1), -1}},
    {"LDX", 1, {IMD(0xa2), ZP(0xa6), ZPY(0xB6), ABS(0xae), ABY(0xbe), -1}},
    {"LDY", 1, {IMD(0xa0), ZP(0xa4), ZPX(0xB4), ABS(0xac), ABX(0xbc), -1}},
    {"ORA",
     1,
     {IMD(0x09), ZP(0x05), ZPX(0x15), ABS(0x0D), ABX(0x1d), ABY(0x19),
      INX(0x01), INY(0x11), -1}},
    {"SBC",
     1,
     {IMD(0xEB), IMD(0xe9), ZP(0xe5), ZPX(0xf5), ABS(0xeD), ABX(0xfd),
      ABY(0xf9), INX(0xe1), INY(0xf1), -1}},
    {"STA",
     2,
     {ZP(0x85), ZPX(0x95), ABS(0x8D), ABX(0x9d), ABY(0x99), INX(0x81),
      INY(0x91), -1}},
    ...
    */
    {
...
#ifdef FCEU_LOW_RAM
          sprintf(stringo + strlen(stringo), " = $%02X", FCEUI_ReadMem(tmp));
#else
      ...
      case 8:
        tmp = ARead[arg](arg) |
              (ARead[(arg + 1) & 0xffff]((arg + 1) & 0xffff) << 8);
        sprintf(stringo + strlen(stringo), " $%04X", tmp);
        break;
      case 9:
        tmp = (arg + X->X) & 0xFF;
        tmp =
            ARead[tmp](tmp) | (ARead[(tmp + 1) & 0xFF]((tmp + 1) & 0xFF) << 8);
        sprintf(stringo + strlen(stringo), "  @ $%04X", tmp);
        if (optable[x].type & 1)
          sprintf(stringo + strlen(stringo), " = $%02X", ARead[tmp](tmp));
        break;
      case 10:
        tmp =
            ARead[arg](arg) | (ARead[(arg + 1) & 0xFF]((arg + 1) & 0xFF) << 8);
        tmp = (tmp + X->Y) & 0xFFFF;
        sprintf(stringo + strlen(stringo), "  @ $%04X", tmp);
        if (optable[x].type & 1)
          sprintf(stringo + strlen(stringo), " = $%02X", ARead[tmp](tmp));
...
else sprintf(stringo + strlen(stringo), "      .db $%02X", buf);
fceuindbg = 0;
return (a);
}
    ...
    if (w == x) {
      if (prev) {      /* Not the first breakpoint. */
        ...
        if (tmp->next) /* More breakpoints. */
          ...
          X6502_Debug(CPUHook, BreakPoints ? ReadHandler : 0,
                      BreakPoints ? WriteHandler : 0);
        }
      }
      free(tmp);
      tmp = 0;
