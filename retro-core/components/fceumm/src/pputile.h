...
if (X1 >= 2) {
  ...
  // Fast 32-bit pixel pipeline: 4 pixels per write
  uint32 p1 = (uint32)S[pixdata & 0xF];
  p1 |= (uint32)S[(pixdata >> 4) & 0xF] << 8;
  p1 |= (uint32)S[(pixdata >> 8) & 0xF] << 16;
  p1 |= (uint32)S[(pixdata >> 12) & 0xF] << 24;
  *(uint32 *)P = p1;

  uint32 p2 = (uint32)S[(pixdata >> 16) & 0xF];
  p2 |= (uint32)S[(pixdata >> 20) & 0xF] << 8;
  p2 |= (uint32)S[(pixdata >> 24) & 0xF] << 16;
  p2 |= (uint32)S[(pixdata >> 28) & 0xF] << 24;
  *(uint32 *)(P + 4) = p2;
...
#else
cc = C[0x3c0 + (zz >> 2) +
       ((RefreshAddr & 0x380) >> 4)]; /* Fetch attribute table byte. */
...
#ifdef PPUT_MMC5CHR1
C = MMC5HackVROMPTR;
C += (((MMC5HackExNTARAMPtr[RefreshAddr & 0x3ff]) & 0x3f & MMC5HackVROMMask)
      << 12) +
     (vadr & 0xfff);
C += (MMC50x5130 & 0x3) << 18;
