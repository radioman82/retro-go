...
void FCEU_PowerCheats() {
  numsubcheats =
      0; /* Quick hack to prevent setting of ancient read addresses. */
  RebuildSubCheats();
}

static int AddCheatEntry(char *name, uint32 addr, uint8 val, int compare,
                         int status, int type);
...
int FCEUI_AddCheat(const char *name, uint32 addr, uint8 val, int compare,
                   int type) {
  char *t;
...
int FCEUI_DelCheat(uint32 which) {
  ...
  for (prev = 0, cur = cheats;;) {
    ...
    if (!cur->next) /* No more cheats to go through(this shouldn't ever
                       happen...) */
      return (0);
    prev = cur;
    cur = prev->next;
...
void FCEUI_ListCheats(int (*callb)(char *name, uint32 a, uint8 v, int compare,
                                   int s, int type, void *data),
                      void *data) {
  struct CHEATF *next = cheats;

  while (next) {
    if (!callb(next->name, next->addr, next->val, next->compare, next->status,
               next->type, data))
      break;
    next = next->next;
  }
...
int FCEUI_GetCheat(uint32 which, char **name, uint32 *a, uint8 *v, int *compare,
                   int *s, int *type) {
  struct CHEATF *next = cheats;
  uint32 x = 0;
...
static int GGtobin(char c) {
  static char lets[16] = {'A', 'P', 'Z', 'L', 'G', 'I', 'T', 'Y',
                          'E', 'O', 'X', 'U', 'K', 'S', 'V', 'N'};
  int x;
...
int FCEUI_DecodePAR(const char *str, uint16 *a, uint8 *v, int *c, int *type) {
  ...
  *a = boo[2] | (boo[1] << 8);
  ...
  /* Zero-page addressing modes don't go through the normal read/write handlers
     in FCEU, so we must do the old hacky method of RAM cheats.
  */
  if (*a < 0x0100)
    *type = 0;
  else
...
int FCEUI_SetCheat(uint32 which, const char *name, int32 a, int32 v,
                   int compare, int s, int type) {
  struct CHEATF *next = cheats;
  uint32 x = 0;
...
/* This function will give the initial value of the search and the current value
 * at a location. */
...
void FCEUI_CheatSearchGetRange(uint32 first, uint32 last,
                               int (*callb)(uint32 a, uint8 last,
                                            uint8 current)) {
  uint32 x;
  uint32 in = 0;
...
void FCEUI_CheatSearchEnd(int type, uint8 v1, uint8 v2) {
  ...
  if (!type) { /* Change to a specific value. */
    ...
    for (x = 0; x < 0x10000; x++)
      if (!(CheatComp[x] & CHEATC_NOSHOW)) {
        if (CheatComp[x] == v1 &&
            CAbs(CheatComp[x] - CheatRPtrs[x >> 10][x]) == v2) {
        } else
          CheatComp[x] |= CHEATC_EXCLUDED;
      }
