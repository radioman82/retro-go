...
extern void lynx_decrypt(unsigned char *result, const unsigned char *encrypted,
                         const int length);
...
CSystem::CSystem(const UBYTE *filedata, ULONG filesize, long displayformat,
                 long samplerate) {
...
void CSystem::Init(const UBYTE *filedata, ULONG filesize, long displayformat,
                   long samplerate) {
  ...
  if (!filedata || filesize < 16) {
    ...
    mFileType =
        memcmp(filedata, "LYNX", 4) ? HANDY_FILETYPE_RAW : HANDY_FILETYPE_LNX;
  ...
  // Now init is complete do a reset, this will cause many things to be reset
  // twice but what the hell, who cares, I don't.....
...
void CSystem::Reset(void) {
  ...
  //	memset(gAudioBuffer,128,HANDY_AUDIO_BUFFER_SIZE); // only for unsigned
  //8bit
...
bool CSystem::ContextSave(LSS_FILE *fp) {
  ...
  // fp->index = 0;
  if (!lss_printf(fp, LSS_VERSION))
    status = 0;

  // Save ROM CRC
  ULONG checksum = mCart->CRC32();
  if (!lss_write(&checksum, sizeof(ULONG), 1, fp))
    status = 0;

  if (!lss_printf(fp, "CSystem::ContextSave"))
    status = 0;

  if (!lss_write(&mCycleCountBreakpoint, sizeof(ULONG), 1, fp))
    status = 0;
  if (!lss_write(&gSystemCycleCount, sizeof(ULONG), 1, fp))
    status = 0;
  if (!lss_write(&gNextTimerEvent, sizeof(ULONG), 1, fp))
    status = 0;
  if (!lss_write(&gCPUWakeupTime, sizeof(ULONG), 1, fp))
    status = 0;
  if (!lss_write(&gCPUBootAddress, sizeof(ULONG), 1, fp))
    status = 0;
  if (!lss_write(&gIRQEntryCycle, sizeof(ULONG), 1, fp))
    status = 0;
  if (!lss_write(&gBreakpointHit, sizeof(ULONG), 1, fp))
    status = 0;
  if (!lss_write(&gSingleStepMode, sizeof(ULONG), 1, fp))
    status = 0;
  if (!lss_write(&gSystemIRQ, sizeof(ULONG), 1, fp))
    status = 0;
  if (!lss_write(&gSystemNMI, sizeof(ULONG), 1, fp))
    status = 0;
  if (!lss_write(&gSystemCPUSleep, sizeof(ULONG), 1, fp))
    status = 0;
  if (!lss_write(&gSystemCPUSleep_Saved, sizeof(ULONG), 1, fp))
    status = 0;
  if (!lss_write(&gSystemHalt, sizeof(ULONG), 1, fp))
    status = 0;
  if (!lss_write(&gThrottleMaxPercentage, sizeof(ULONG), 1, fp))
    status = 0;
  if (!lss_write(&gThrottleLastTimerCount, sizeof(ULONG), 1, fp))
    status = 0;
  if (!lss_write(&gThrottleNextCycleCheckpoint, sizeof(ULONG), 1, fp))
    status = 0;

  if (!lss_write(&gTimerCount, sizeof(ULONG), 1, fp))
    status = 0;

  if (!lss_write(&gAudioLastUpdateCycle, sizeof(ULONG), 1, fp))
    status = 0;

  if (!lss_write(&mMemMapReg, sizeof(UBYTE), 1, fp))
    status = 0;
...
bool CSystem::ContextLoad(LSS_FILE *fp) {
  ...
  if (strcmp(teststr, LSS_VERSION) == 0) {
    ...
    if (mCart->CRC32() != checksum) {
      log_printf("CSystem::ContextLoad() LSS Snapshot CRC does not match the "
                 "loaded cartridge image...\n");
      // return 0;
    }
    ...
    if (!lss_read(&mCycleCountBreakpoint, sizeof(ULONG), 1, fp))
      status = 0;
    if (!lss_read(&gSystemCycleCount, sizeof(ULONG), 1, fp))
      status = 0;
    if (!lss_read(&gNextTimerEvent, sizeof(ULONG), 1, fp))
      status = 0;
    if (!lss_read(&gCPUWakeupTime, sizeof(ULONG), 1, fp))
      status = 0;
    if (!lss_read(&gCPUBootAddress, sizeof(ULONG), 1, fp))
      status = 0;
    if (!lss_read(&gIRQEntryCycle, sizeof(ULONG), 1, fp))
      status = 0;
    if (!lss_read(&gBreakpointHit, sizeof(ULONG), 1, fp))
      status = 0;
    if (!lss_read(&gSingleStepMode, sizeof(ULONG), 1, fp))
      status = 0;
    if (!lss_read(&gSystemIRQ, sizeof(ULONG), 1, fp))
      status = 0;
    if (!lss_read(&gSystemNMI, sizeof(ULONG), 1, fp))
      status = 0;
    if (!lss_read(&gSystemCPUSleep, sizeof(ULONG), 1, fp))
      status = 0;
    if (!lss_read(&gSystemCPUSleep_Saved, sizeof(ULONG), 1, fp))
      status = 0;
    if (!lss_read(&gSystemHalt, sizeof(ULONG), 1, fp))
      status = 0;
    if (!lss_read(&gThrottleMaxPercentage, sizeof(ULONG), 1, fp))
      status = 0;
    if (!lss_read(&gThrottleLastTimerCount, sizeof(ULONG), 1, fp))
      status = 0;
    if (!lss_read(&gThrottleNextCycleCheckpoint, sizeof(ULONG), 1, fp))
      status = 0;

    if (!lss_read(&gTimerCount, sizeof(ULONG), 1, fp))
      status = 0;

    if (!lss_read(&gAudioLastUpdateCycle, sizeof(ULONG), 1, fp))
      status = 0;

    if (!lss_read(&mMemMapReg, sizeof(UBYTE), 1, fp))
      status = 0;
...
void CSystem::UpdateFrame(bool draw) {
  ...
  while (gEndOfFrame != TRUE) {
    ...
    if (lastcycle < mCycleCountBreakpoint &&
        gSystemCycleCount >= mCycleCountBreakpoint)
      gBreakpointHit = TRUE;
  ...
  if (address) {
    if (address == 0xffff) {
      ...
      log_printf(linetext,
                 ...
                 regs.PC, regs.SP, regs.PS, regs.A, regs.X, regs.Y);
      strcat(message, linetext);
      count = strlen(message);
    } else {
      // The RAM address contents should be dumped to an open debug file in this
      // function
      do {
...
void CSystem::DebugSetCallback(void (*function)(ULONG objref, char *message),
                               ULONG objref) {
