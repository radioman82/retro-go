...
rom_t *rom_loadmem(uint8 *data, size_t size) {
  ...
  if (!memcmp(data, ROM_NES_MAGIC, 4)) {
    ...
    rom.mapper_number =
        ((header->mapper_hinybble & 0xF0) | (header->rom_type >> 4));
    // https://wiki.nesdev.com/w/index.php/INES
    // A general rule of thumb: if the last 4 bytes are not all zero, and the
    // header is not marked for NES 2.0 format, an emulator should either mask
    // off the upper 4 bits of the mapper number or simply refuse to load the
    // ROM.
    ...
    rom.checksum =
        CRC32(0, rom.data_ptr + data_offset, rom.data_len - data_offset);
    ...
    size_t expected_size = data_offset +
                           (rom.prg_rom_banks * ROM_PRG_BANK_SIZE) +
                           (rom.chr_rom_banks * ROM_CHR_BANK_SIZE);
    ...
    if (rom.chr_rom_banks > 0)
      rom.chr_rom =
          data + data_offset + (rom.prg_rom_banks * ROM_PRG_BANK_SIZE);
  } else if (!memcmp(data, ROM_FDS_MAGIC, 4) ||
             !memcmp(data, ROM_FDS_RAW_MAGIC, 15)) {
    ...
    rom.battery = true;
    rom.prg_ram_banks =
        4; // The FDS adapter contains 32KB to store game program
    ...
    rom.prg_ram_banks = 1; // Some songs may need it. I store a bootstrap
                           // program there at the moment
    rom.chr_ram_banks =
        1; // Not used but some code might assume it will be present...
    rom.prg_rom_banks = 4; // This is actually PRG-RAM but some of our code
                           // assumes PRG-ROM to be present...
    rom.checksum = CRC32(0, rom.data_ptr, rom.data_len);
  } else {
  ...
  MESSAGE_INFO("ROM: Mapper: %d, PRG:%dK, CHR:%dK, Flags: %c%c%c%c\n",
               rom.mapper_number, rom.prg_rom_banks * 8, rom.chr_rom_banks * 8,
               (rom.vertical) ? 'V' : 'H', (rom.battery) ? 'B' : '-',
               (rom.trainer) ? 'T' : '-', (rom.fourscreen) ? '4' : '-');

  rom.prg_ram = rg_alloc(rom.prg_ram_banks * ROM_PRG_BANK_SIZE, MEM_SLOW);
  rom.chr_ram = rg_alloc(rom.chr_ram_banks * ROM_CHR_BANK_SIZE, MEM_SLOW);
  if (!rom.prg_rom) {
    rom.prg_rom = rg_alloc(rom.prg_rom_banks * ROM_PRG_BANK_SIZE, MEM_SLOW);
...
rom_t *rom_loadfile(const char *filename) {
  ...
  if ((fp = fopen(filename, "rb"))) {
    ...
    } else if ((data = rg_alloc(size, MEM_SLOW)) == NULL) {
      MESSAGE_ERROR("ROM: Memory allocation failed\n");
  ...
  if (rom.system == SYS_UNKNOWN) {
    if (strstr(filename, "(E)") || strstr(filename, "(Europe)") ||
