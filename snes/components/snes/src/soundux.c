...
#define CLIP8(v) (v) = (((v) <= -128) ? -128 : (((v) >= 127) ? 127 : (v)))
...
static const uint32_t AttackRate[16] = {
    4100, 2600, 1500, 1000, 640, 380, 260, 160, 96, 64, 40, 24, 16, 10, 6, 1};

static const uint32_t DecayRate[8] = {1200, 740, 440, 290, 180, 110, 74, 37};
...
static const uint32_t IncreaseRate[32] = {
    0xFFFFFFFF, 4100, 3100, 2600, 2000, 1500, 1300, 1000, 770, 640, 510,
    380,        320,  260,  190,  160,  130,  96,   80,   64,  48,  40,
    32,         24,   20,   16,   12,   10,   8,    6,    4,   2};
...
void S9xAPUSetEndX(int32_t ch) { APU.DSP[APU_ENDX] |= 1 << ch; }

void IRAM_ATTR S9xSetEnvRate(Channel *ch, uint32_t rate, int32_t direction,
                             int32_t target, uint32_t mode) {
...
void S9xSetEnvelopeRate(int32_t channel, uint32_t rate, int32_t direction,
                        int32_t target, uint32_t mode) {
  S9xSetEnvRate(&SoundData.channels[channel], rate, direction, target, mode);
}

void S9xSetSoundVolume(int32_t channel, int16_t volume_left,
...
void S9xFixSoundAfterSnapshotLoad() {
  ...
  for (i = 0; i < 8; i++) {
    ...
    SoundData.channels[i].envxx = SoundData.channels[i].envx << ENVELOPE_SHIFT;

    // Fix pointers corrupted by snapshot load
    SoundData.channels[i].block = SoundData.channels[i].decoded;
    if (SoundData.echo_enable & (1 << i))
      SoundData.channels[i].echo_buf_ptr = EchoBuffer;
    else
      SoundData.channels[i].echo_buf_ptr = NULL;
  }
...
void S9xSetSoundADSR(int32_t channel, int32_t attack_ind, int32_t decay_ind,
                     int32_t sustain_ind, int32_t sustain_level,
...
void IRAM_ATTR DecodeBlock(Channel *ch) {
  int32_t out;
  uint8_t filter;
  uint8_t shift;
  ...
  case 3:
    for (i = 8; i != 0; i--) {
      ...
      prev0 &= ~3;
      *raw++ = prev0 =
          out + (prev0 << 1) - (prev0 >> 3) - (prev0 >> 4) - (prev1 >> 6);
      out = (sample2 << shift);
      out = out - prev1 + (prev1 >> 3) + (prev1 >> 4);
      prev1 = (int16_t)prev0;
      prev0 &= ~3;
...
static INLINE void IRAM_ATTR MixStereo(int32_t sample_count) {
  ...
  for (J = 0; J < NUM_CHANNELS; J++) {
    ...
    if (ch->needs_decode) {
      ...
      if (Settings.InterpolatedSound && freq0 < FIXED_POINT && !mod)
        ch->interpolate = ((ch->next_sample - ch->sample) * (int32_t)freq0) /
                          (int32_t)FIXED_POINT;
    }
      ...
      if (ch->count >= FIXED_POINT) {
        ...
        if (ch->type == SOUND_SAMPLE) {
          if (Settings.InterpolatedSound && freq < FIXED_POINT && !mod) {
            ch->interpolate = ((ch->next_sample - ch->sample) * (int32_t)freq) /
                              (int32_t)FIXED_POINT;
            ch->sample =
                (int16_t)(ch->sample + (((ch->next_sample - ch->sample) *
                                         (int32_t)(ch->count)) /
                                        (int32_t)FIXED_POINT));
          } else
            ch->interpolate = 0;
  ...
  if (SoundData.echo_enable && SoundData.echo_buffer_size) {
    ...
    if (FilterTapDefinitionBitfield == 0) {
      ...
      for (J = 0; J < sample_count; J++) {
        ...
        (*low_pass_sample) =
            ((*low_pass_sample) * low_pass_factor_a) + (I * low_pass_factor_b);
        /* 16.16 fixed point */
        (*low_pass_sample) >>= 16;
...
void S9xSetPlaybackRate(uint32_t playback_rate) {
  ...
  if (playback_rate) {
    ...
    /* notaz: calculate a value (let's call it freqbase) to simplify channel
     * freq calculations later. */
    ...
    for (u = 0; u < 10; u++) {
      ...
      for (i = 0; i < 16; i++)
        AttackERate[i][u] =
            (uint32_t)(fp1000su / (AttackRate[i] * playback_rate));

      for (i = 0; i < 8; i++)
        DecayERate[i][u] =
            (uint32_t)(fp1000su / (DecayRate[i] * playback_rate));

      for (i = 0; i < 32; i++) {
        SustainERate[i][u] =
            (uint32_t)(fp1000su / (SustainRate[i] * playback_rate));
        IncreaseERate[i][u] =
            (uint32_t)(fp1000su / (IncreaseRate[i] * playback_rate));
        DecreaseERateExp[i][u] =
            (uint32_t)(fp1000su / (DecreaseRateExp[i] / 2 * playback_rate));
      }
