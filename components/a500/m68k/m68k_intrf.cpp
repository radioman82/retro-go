...
static unsigned timeslice_shift=7;
int next_positions[512];
int *next_vpos=&next_positions[0];

void init_default_vpos(void)
{
	for (int i = 0; i < 512; i++)
		next_positions[i] = i + 1;
	next_positions[511] = 0;
}
...
static void a500_m68k_log(const char *fmt, ...) {
    char buf[256];
    va_list va;
    va_start(va, fmt);
...
static void uae4all_reset(void)
{
    ...
    a500_m68k_log("uae4all_reset complete: PC=0x%08X, SP=0x%08X", _68k_getpc(), _68k_areg(7));
...
//			M68KCONTEXT.interrupts[0]=(M68KCONTEXT.interrupts[0]&1)|uae4all_go_interrupt;
		...
		if (cycles_factor == 1.0)
			...
			cycles = ((unsigned)((float)(M68KCONTEXT.cycles_counter - cycles_actual) * (float)cycles_factor)) << 8;
